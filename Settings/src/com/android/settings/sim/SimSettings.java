...
public class SimSettings extends RestrictedSettingsFragment implements Indexable,
    ...
    private void updateCallValues() {
        ...
        if (simPref != null) {
            ...
            //lijingwei@wind-mobi.com Bug#100768 2016/3/17 start
            Log.d(TAG, "updateCallValues phoneAccount=" + phoneAccount);
            //lijingwei@wind-mobi.com Bug#100768 2016/3/17 end
            ...
            //lijingwei@wind-mobi.com Bug#100768 2016/6/15 start
            Log.d(TAG, "updateCallValues mSelectableSubInfos.size()=" + mSelectableSubInfos.size());
            if(defaultAccount == null && mSelectableSubInfos.size() == 1){
                SubscriptionInfo sir = mSubscriptionManager.getDefaultSmsSubscriptionInfo();
                /// M: for plug-in
                sir = mSimManagementExt.setDefaultSubId(getActivity(), sir, KEY_CALLS);
                if (sir != null) {
                    //fengsong@wind-mobi.com 20160725 add start
                    //modify sim name to life:)
                    if(FeatureOption.WIND_DEF_OPTR_E183L_LIFE
                    ...
                    //fengsong@wind-mobi.com 20160725 add end
                    Log.d(TAG, "updateCallValues sir.getDisplayName()=" + sir.getDisplayName());
                    simPref.setSummary(sir.getDisplayName());
                } else if (sir == null) {
                    simPref.setSummary(R.string.sim_calls_ask_first_prefs_title);
                }
            }else
            //lijingwei@wind-mobi.com Bug#100768 2016/6/15 end
            simPref.setSummary(defaultAccount == null
                    ? mContext.getResources().getString(R.string.sim_calls_ask_first_prefs_title)
                    : (String)defaultAccount.getLabel());
            //fengsong@wind-mobi.com 20160725 add start
            //modify sim name to life:)
            if(FeatureOption.WIND_DEF_OPTR_E183L_LIFE && (defaultAccount != null)
                && (String.valueOf((String)defaultAccount.getLabel()).contains("BelarusianTelecommunicationsNet"))){
                simPref.setSummary("life:)");
            }
            //fengsong@wind-mobi.com 20160725 add end
            /// @}
            /// M: [C2K solution 2 enhancement] [C2K solution 1.5] @{
            if (!shouldDisableActivitesCategory(getActivity())) {
                //lijingwei@wind-mobi.com Bug #102833 2016/3/25 start
                //simPref.setEnabled(allPhoneAccounts.size() > 1);
                simPref.setEnabled(allPhoneAccounts.size() >= 1);
                //lijingwei@wind-mobi.com Bug #102833 2016/3/25 end
    ...
    private class SimPreference extends RadioPowerPreference{
        ...
        public void update() {
            ...
            if (mSubInfoRecord != null) {
                ...
                //modify sim name to life:)
                if(FeatureOption.WIND_DEF_OPTR_E183L_LIFE
                    && (String.valueOf(mSubInfoRecord.getDisplayName()).contains("BelarusianTelecommunicationsNet"))){
                    mSubInfoRecord.setDisplayName("life:)");
                }
                //fengsong@wind-mobi.com 20160725 add end
