...
//        FIXME Waiting for telephonyprovider ready
    ...
    private void updateApn(Context context, Uri uri, String apn, String apnId,
        ...
        //lijingwei@wind-mobi.com 2016.08.18 start
        //fix Feature #122202 OTA update
        //long replaceNum = replaceApn(context, peferredUri, apn, apnId, name, values, numeric);
        long replaceNum = -2;
        if (FeatureOption.WIND_DEF_OPTR_E183L_EUR) {
            replaceNum = replaceApn(context, uri, apn, apnId, name, values, numeric);
        }else {
            replaceNum = replaceApn(context, peferredUri, apn, apnId, name, values, numeric);
        }
        //lijingwei@wind-mobi.com 2016.08.18 end
        /// M: for plug-in
        ...
        //fix Feature #122202 OTA update
        if (FeatureOption.WIND_DEF_OPTR_E183L_EUR) {
            Log.d(TAG, "updateApn for Wind_E183L_EUR");
            if (replaceNum == APN_NO_UPDATE) {
                ...
                values = addMVNOItem(values);
                try {
                    Uri updateUri = ContentUris.withAppendedId(uri, replaceNum);
                    context.getContentResolver().update(updateUri, values, null, null);
                } catch (SQLException e) {
                    Log.d(TAG, "update SQLException happened!");
                    mResult = false;
                }
            }
        } else {
        ...
        if (FeatureOption.WIND_DEF_OPTR_E183L_EUR) {
            SystemProperties.set("persist.sys.newapnid", String.valueOf(insertNum));
        }
        //lijingwei@wind-mobi.com 2016.07.26 end
    ...
    public long replaceApn(Context context, Uri uri, String apn, String apnId, String name,
        ...
        try {
            ...
            //fix Feature #122202 OTA update
            if (FeatureOption.WIND_DEF_OPTR_E183L_EUR) {
                Log.d(TAG, "replace apn for Wind_E83L_EUR uri=" + uri);
                cursor = context.getContentResolver().query(uri,
                        new String[] {
                                Telephony.Carriers._ID, Telephony.Carriers.OMACPID,
            ...
            while (!cursor.isAfterLast()) {
                ...
                //fix Feature #122202 OTA update
                if (FeatureOption.WIND_DEF_OPTR_E183L_EUR) {
                    Log.d(TAG, "name " + name + " getName = " + cursor.getString(2));
